...
public class GmCBCAbstractMessageAlgorithm extends AbstractMessageAlgorithm {
    ...
    public byte[] symmetricEncryption(byte[] secretKey, byte[] data, byte[] iv) {
        ...
        try {
            return SM4Util.encrypt_CBC_NoPadding(secretKey, iv, padding);
        } catch (Exception ex) {
            throw new RuntimeException("GmCBCAbstractMessageAlgorithm symmetricEncryption error ", ex);
        }
    }
    ...
    public byte[] symmetricDecrypt(byte[] secretKey, byte[] data, byte[] iv) {
        try {
            byte[] bytes = SM4Util.decrypt_CBC_NoPadding(secretKey, iv, data);
